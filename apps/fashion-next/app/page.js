...
export default function Home() {
  ...
  const { query, setQuery, results, setResults, loading, error, performSearch } = useSearch(
  ...
  useEffect(() => {
    ...
    if (urlQuery && urlQuery.trim() && !hasLoadedFromUrl.current) {
      ...
      // Clear wardrobeItems from localStorage when no search query in URL
      localStorage.removeItem("wardrobeItems");
      setResults([]); // Clear search results when URL query is removed
    }
  ...
  const handleSearch = (searchQuery) => {
    ...
    console.log("handleSearch called with:", { searchQuery, query });
